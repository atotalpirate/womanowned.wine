...
( function( $ ) {
	$( document ).ready( function() {
		...
		function fadeInLastImg( className ) {
			const backImg = $( '.' + className + '-masthead-images image:first' );
			backImg.hide();
		...
		const modalIsActive = document.getElementsByClassName( 'modal is-active' );
		...
		if ( window.location.search || ( window.location.pathname.indexOf( '/wineries/' ) !== -1 ) ) {
			const subscribeWall = document.getElementById( 'subscribeWall' ),
		...
		function submitSubscribeForm( $form, $resultElement ) {
			$.ajax( {
				...
				error( error ) {
					...
					const message = error.msg || 'Sorry. Unable to subscribe. Please try again later.';
					$resultElement.html( message );
				},
				success( data ) {
					if ( data.result != 'success' ) {
						const fullMessage = data.msg.replace( '0 -', '' ) || data.result;
